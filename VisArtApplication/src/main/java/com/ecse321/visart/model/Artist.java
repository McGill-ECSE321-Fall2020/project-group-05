...
public class Artist {
  ...
  public Artist(String aIdCode, Customer aCustomer) {
    ...
    if (!didAddCustomer) {
      throw new RuntimeException(
          "Unable to create artist due to customer. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
  ...
  public Ticket addSoldTicket(boolean aIsPaymentConfirmed, double aPaymentAmount, String aIdCode,
      ArtOrder aOrder, Customer aCustomer) {
  ...
  public boolean addSoldTicketAt(Ticket aSoldTicket, int index) {
    ...
    if (addSoldTicket(aSoldTicket)) {
      ...
      if (index > numberOfSoldTickets()) {
        index = numberOfSoldTickets() - 1;
      }
      soldTickets.remove(aSoldTicket);
  ...
  /* Code from template association_AddManyToOne */
  public ArtListing addPostedListing(ArtListing.PostVisibility aVisibility, String aDescription,
  ...
  public boolean addPostedListingAt(ArtListing aPostedListing, int index) {
    ...
    if (addPostedListing(aPostedListing)) {
      ...
      if (index > numberOfPostedListings()) {
        index = numberOfPostedListings() - 1;
      }
      postedListings.remove(aPostedListing);
  ...
  public boolean setCustomer(Customer aNewCustomer) {
    ...
    if (aNewCustomer == null) {
      // Unable to setCustomer to null, as artist must always be associated to a
      // customer
      return wasSet;
    }
    ...
    if (existingArtist != null && !equals(existingArtist)) {
      // Unable to setCustomer, the current customer already has a artist, which would
