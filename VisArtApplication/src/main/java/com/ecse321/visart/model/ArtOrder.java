...
public class ArtOrder {
  ...
  public ArtOrder(boolean aIsDelivered, PieceLocation aTargetLocation, String aTargetAddress,
      String aDeliveryTracker, String aIdCode, ArtPiece aArtPiece) {
    ...
    if (!didAddArtPiece) {
      throw new RuntimeException(
          "Unable to create artOrder due to artPiece. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
  ...
  public boolean setArtPiece(ArtPiece aNewArtPiece) {
    ...
    if (aNewArtPiece == null) {
      // Unable to setArtPiece to null, as artOrder must always be associated to a
      // artPiece
      return wasSet;
    }
    ...
    if (existingArtOrder != null && !equals(existingArtOrder)) {
      // Unable to setArtPiece, the current artPiece already has a artOrder, which
  ...
  public String toString() {
    return super.toString() + "[" +
        ...
        "  " + "targetLocation" + "="
        + (getTargetLocation() != null ? !getTargetLocation().equals(this)
            ? getTargetLocation().toString().replaceAll("  ", "    ")
            : "this" : "null")
        + System.getProperties().getProperty("line.separator") +
        "  " + "artPiece = "
        + (getArtPiece() != null ? Integer.toHexString(System.identityHashCode(getArtPiece()))
            : "null")
        + System.getProperties().getProperty("line.separator") +
        "  " + "ticket = "
        + (getTicket() != null ? Integer.toHexString(System.identityHashCode(getTicket()))
            : "null");
  }
}