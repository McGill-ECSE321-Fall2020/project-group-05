...
public class UserRole {
  ...
  public UserRole(String aIdCode, User aUser) {
    ...
    if (!didAddUser) {
      throw new RuntimeException(
          "Unable to create role due to user. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
  ...
  public boolean setUser(User aNewUser) {
    ...
    if (existingRole != null && !equals(existingRole)) {
      // Unable to setUser, the current user already has a role, which would be
